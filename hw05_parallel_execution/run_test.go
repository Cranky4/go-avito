...
func TestRun(t *testing.T) {
	...
	t.Run("run with zero errors parameter as error response", func(t *testing.T) {
		tasksCount := 10
		tasks := make([]Task, 0, tasksCount)
	...
	t.Run("tasks were run sequentially?", func(t *testing.T) {
		...
		assert.Eventuallyf(t, func() bool {
			...
			return goroutinesCount >= 1
		}, time.Millisecond*100, time.Millisecond*10, "tasks were run sequentially? running: %d", goroutinesCount)
		err := Run(tasks, workersCount, maxErrorsCount)
		require.NoError(t, err)
